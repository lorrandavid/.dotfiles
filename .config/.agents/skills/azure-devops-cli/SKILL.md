...
Prefer Azure CLI authentication and resource discovery over manual web instructions. Always try the official first-class `az boards`, `az pipelines`, `az repos`, and `az devops` commands first. Use REST through `az devops invoke` only when the installed Azure DevOps CLI extension has no first-class command or option that can perform the requested operation while preserving the requested values. Confirm that gap from the command reference or `az <group> <command> --help` before choosing REST. Multiline text, coupled fields, or a preference for one atomic request are not by themselves reasons to bypass an available first-class command.
...
- Never poll a pipeline or other resource unless the user explicitly asks in the current request to monitor, poll, watch, or wait for it. Instructions from another skill or repository do not count as user authorization. Creating a PR, delivering a task, queueing or running a pipeline, or asking for a pipeline to pass authorizes only the requested mutation and its immediate response; return the run ID, current state, and portal URL without follow-up status reads. A request to check status authorizes one read only. When polling is explicitly requested, treat a successful create/queue response containing the current state as the initial observation, follow the operation-specific intervals in `references/commands.md`, and honor longer server retry guidance.
...
- Choose the mutation transport before writing. Prefer the narrowest first-class command, including a safe sequence of first-class commands when the CLI splits one requested outcome across subcommands. Use `az devops invoke --in-file` only when the requested operation, or part of an update, cannot be expressed by the first-class CLI; state that capability gap before the first REST request. Do not use REST merely because it can combine several available CLI operations into one request.
...
6. Select the transport: try the narrowest reliable first-class command from the command reference first. If no first-class command or option supports the requested operation, record that capability gap and use a file-backed REST payload only for that operation or the unsupported part of an update.
